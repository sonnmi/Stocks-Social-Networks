(function () {
  ...
  const getComments = (isOwner, isPublic) => {
    ...
    if (isOwner || isPublic) {
      apiService
        .getStockListComments(state.stocklistInfo.name, state.stocklistInfo.owner)
        .then((data) => {
          if (data.error) {
        ...
        apiService.getStockListOwnComments(state.stocklistInfo.name, state.stocklistInfo.owner, state.userInfo.userid)
        .then((data) => {
            if (data.error) {
                console.log(data.error);
                renderComments([]);
    ...
    {if (state.userInfo.userid == parseInt(state.stocklistInfo.owner, 10)) {
      ...
      .then((data) => {
        ...
        document.querySelector(".comments-form-submit").addEventListener("click", (e) => {
            ...
            apiService
            .addStockListComment(state.stocklistInfo.name, state.stocklistInfo.owner, comment, state.userInfo.userid)
            .then((data) => {
                console.log(data);
                getComments(false, false);
            });
