(function () {
  ...
  const updatePortfolio = () => {
    const username = JSON.parse(localStorage.getItem("userInfo")).username;
    apiService.getPortfoliosOfUser(username).then((response) => {
      if (response.error) {
        ...
        if (state.portfolios.length > 0) {
          state.portfolios.map((portfolio) => {
            const newPortf = createPortfolioComponent(portfolio);
            console.log(newPortf);
            document
              .querySelector(".card.portfolio-container")
  ...
  window.addEventListener("load", function (event) {
    ...
    document
      .querySelector(".add-portfolio-form .btn")
