(function () {
    ...
    function sendFriendRequest() {
        ...
        apiService.sendRequest(state.userInfo.username, friendUsername, "friend", "pending", new Date());
    }
    ...
    window.addEventListener("load", function (event) {
        ...
        this.document.querySelector(".profile .username").innerHTML = state.userInfo.username;
        getFriends();
        getFriendRequests();
        ...
        friendsList.addEventListener("click", function (event) {
            if (event.target.classList.contains("remove-friend-btn")) {
                ...
                apiService.updateRequest("deleted", state.userInfo.username, friendName).then(() => {
                    apiService.deleteFriend(state.userInfo.username, friendName).then(() => {
                        getFriends();
                    });
                });
            }
        ...
        friendRequests.addEventListener("click", function (event) {
            if (event.target.classList.contains("accept-friend-btn")) {
                ...
                apiService.updateRequest("accepted", requestSender, state.userInfo.username).then(() => {
                    apiService.addFriend(state.userInfo.username, requestSender).then(() => {
                        getFriends();
                        getFriendRequests();
                    });
                });
                ...
                const requestElement = event.target.closest(".request-item");
                const requestSender = requestElement.querySelector(".friend-name").innerHTML;
                apiService.updateRequest("rejected", requestSender, state.userInfo.username).then(() => {
                    getFriendRequests();
                });
            }
        });
